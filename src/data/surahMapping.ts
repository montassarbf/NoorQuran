const slugMap: Record<string, number> = {
  ...
  'al-an-am': 6, 'al-araf': 7, 'al-anfal': 8, 'at-tawbah': 9, 'yunus': 10,
...
export function getSurahNumber(slug: string): number | undefined {
  return slugMap[slug.toLowerCase()];
}

export function getSurahSlug(surahNumber: number): string {
  const entries = Object.entries(slugMap);
  for (const [slug, num] of entries) {
    if (num === surahNumber) return slug;
  }
  return '';
}
